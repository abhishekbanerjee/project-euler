fn main() {
    let product = largest_product(13);
    println!("{}", product);
}
...
fn largest_product(to_consider: usize) -> u64 {
	...
	if considered > to_consider {
	    current /= digits[idx-to_consider];
	    considered -= 1;
	}
	// The update condition. Since we're only interested in groups
	// of 13 digits exactly.
	if considered == to_consider && current > largest {
	    largest = current;
	}
    }
    largest
